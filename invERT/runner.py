...
def init(config: Config, metadata_size: int, hidden_layers: list[int], num_filters: list[int], kernel_sizes: list[int], in_channels: int, device: str) -> tuple[DynamicModel, Optimizer, Module]:
    model = DynamicModel(metadata_size, hidden_layers, num_filters, kernel_sizes, in_channels)
...
def main(config: Config):
    ...
    data, target, max_input_shape, min_data, max_data, min_target, max_target = pre_process_data(data)
    ...
    for conv_layer in cnn_config.conv_layers:
        num_filters.append(conv_layer.filters) # /!\ last filter number must be 1
    ...
    for repetition in range(config.experiment.repetitions):
        ...
        model, optimizer, scheduler = init(config, mlp_config.input_size, mlp_config.hidden_layers, num_filters, kernel_sizes, cnn_config.input_channels, device)
        loss_array[repetition], test_loss_array[repetition], model = train(model, epochs, train_dataloader, test_dataloader, optimizer, criterion, scheduler, loss_array[repetition], test_loss_array[repetition], max_input_shape, device, print_points)
    ...
    print_model_results(model_list, val_dataloader, device, max_input_shape, min_data, max_data, min_target, max_target)
